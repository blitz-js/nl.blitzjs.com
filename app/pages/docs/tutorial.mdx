...
Als je een foutmelding krijgt, voer dan `blitz prisma format` uit.

Merk op dat dit geen databasemigratie vereist omdat we het `Choice` veld
nog niet aan het `Question` model hebben toegevoegd. Dus we kiezen `false`
wanneer er wordt gevraagd om de migratie uit te voeren:
...
Nu kunnen we de migratie uitvoeren om onze database te updaten:

```sh
blitz prisma migrate dev
```

Vul opnieuw een naam in voor de migratie, zoals "add choice":
...
Nu is onze database klaar en er is ook een Prisma client gegenereerd.
Laten we verder gaan en eens met de Prisma client spelen.

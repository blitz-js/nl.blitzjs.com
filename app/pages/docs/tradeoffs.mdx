...
Zoals je weet, maak je bij alles afwegingen. Wat zijn nu juist de afwegingen die je bij Blitz kan maken?

## Framework lock-in {#framework-lock-in}

Elk framework heeft een bepaalde mate van lock-in. Als je jouw app met React bouwt, is deze nauw verweven met React.
...
De Zero-API data layer is uniek voor Blitz. Dus als je overstapt op Next.js, moet je jouw eigen API-code beheren (wat je ook zou moeten doen als je met Next.js zou beginnen, dus je hebt niets verloren). Al je backend query- en mutatielogica kan echter rechtstreeks worden overgezet.

## Next.js upgrades {#next-js-upgrades}

Als je steeds met de nieuwste versie van Next.js wil werken, weet dan dat Blitz iets achter zal lopen, maar niet ver. Gewoonlijk updaten we Blitz binnen een week of minder na een nieuwe Next.js release.
...
Men zegt wel eens "Je zal uiteindelijk GraphQL nodig hebben, dus kan je er best ineens mee beginnen". Hoewel wij geloven dat de meeste apps niet groot genoeg zullen worden om GraphQL te rechtvaardigen, is de vraag naar een API voor meerdere clients een uitstekend punt.
...
- Kan je de RPC-specificatie voor de endpoints hier zien: [RPC-specificatie](./rpc-specification)
...
3. Ten slotte een first-class react-native ondersteuning bieden waardoor je Blitz queries/mutations rechtstreeks in je react-native code kan importeren zodat het net op dezelfde magische wijze werkt als in je huidige web app. Na de 1.0 release zal Brandon zich vooral hierop focussen voor de rest van het jaar.
...
Dit is een nadeel van Next.js in het bijzonder. Next.js is single-threaded, wat betekent dat als je zware backend-verwerkingen uitvoert, 
je kan merken dat alle webverzoeken eronder beginnen te lijden. 
De oplossing hiervoor is om achtergrondverwerking over te hevelen naar andere processen.
